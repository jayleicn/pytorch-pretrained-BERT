...
def load_tvqa_data():
    ...
    assert "s_tokenized_sub_text" in train_data[0]  # stanford tokenizer matched results
...
def main():
    ...
    tvqa_data = load_tvqa_data()
    if args.mode == "qa":
        input_data = mk_qa_input_lines(tvqa_data)
    else:  # sub
        input_data = mk_sub_input_lines(tvqa_data)
